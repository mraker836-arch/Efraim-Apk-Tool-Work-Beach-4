...
object ScanReportExporter {
    ...
    /**
     * Generates an audit report in GitHub-flavored Markdown format.
     */
    fun generateMarkdownReport(scan: ApkScanResult): String {
        val sb = StringBuilder()
        val dateStr = dateFormat.format(Date(scan.completedAt))

        sb.appendLine("# Efraim APK Workbench - Audit Report")
        sb.appendLine()
        sb.appendLine("**Scan ID:** `${scan.scanId}`  ")
        sb.appendLine("**APK Filename:** `${scan.fileInfo.fileName}`  ")
        sb.appendLine("**File Size:** ${scan.fileInfo.fileSize} bytes (${formatBytes(scan.fileInfo.fileSize)})  ")
        sb.appendLine("**Timestamp:** $dateStr  ")
        sb.appendLine("**Status:** `${scan.status.name}`  ")
        scan.errorMessage?.let {
            sb.appendLine("**Notice / Errors:** $it  ")
        }
        sb.appendLine()

        sb.appendLine("## 1. Cryptographic Hashes")
        ...
        sb.appendLine("## 2. Package & Manifest Details")
        sb.appendLine("- **Package Name:** `${scan.manifestInfo.packageName}`")
        sb.appendLine("- **App Label:** `${scan.manifestInfo.appName}`")
        sb.appendLine("- **Version:** `${scan.manifestInfo.versionName}` (`${scan.manifestInfo.versionCode}`)")
        sb.appendLine("- **Min SDK:** API ${scan.manifestInfo.minSdk}")
        sb.appendLine("- **Target SDK:** API ${scan.manifestInfo.targetSdk}")
        sb.appendLine("- **Debuggable:** `${scan.manifestInfo.isDebuggable}`")
        sb.appendLine("- **Allow Backup:** `${scan.manifestInfo.allowBackup}`")
        sb.appendLine("- **Uses Cleartext Traffic:** `${scan.manifestInfo.usesCleartextTraffic}`")
        sb.appendLine()

        sb.appendLine("## 3. Bytecode & Native Libraries")
        sb.appendLine("- **DEX Count:** ${scan.fileInfo.dexCount}")
        sb.appendLine("- **Native Libraries:** ${scan.nativeLibrariesList.size}")
        sb.appendLine("- **ABIs Detected:** ${if (scan.abiCoverage.isEmpty()) "None (Pure Java / DEX)" else scan.abiCoverage.joinToString(", ")}")
        sb.appendLine()

        sb.appendLine("## 4. Declared Permissions (${scan.permissionsList.size})")
        if (scan.permissionsList.isEmpty()) {
            sb.appendLine("*No permissions declared.*")
        } else {
            sb.appendLine("| Permission | Category | Risk Level |")
            sb.appendLine("| :--- | :--- | :--- |")
            scan.permissionsList.forEach { p ->
                sb.appendLine("| `${p.name}` | ${p.protectionCategory.name} | **${p.riskIndicator.name}** |")
            }
        }
        sb.appendLine()

        sb.appendLine("## 5. Security Audit Findings (${scan.securityFindings.size})")
        if (scan.securityFindings.isEmpty()) {
            sb.appendLine("✅ *No deterministic security findings or vulnerabilities identified.*")
        } else {
            scan.securityFindings.forEach { f ->
                sb.appendLine("### [${f.severity.name}] ${f.title}")
                sb.appendLine("- **Category:** ${f.category.name}")
                sb.appendLine("- **Description:** ${f.description}")
                sb.appendLine("- **Evidence:** `${f.evidence}`")
                sb.appendLine("- **Recommendation:** ${f.recommendation}")
        ...
        sb.appendLine("---")
        sb.appendLine("*Generated by Efraim APK Workbench*")
        return sb.toString()
    }
    ...
    fun generateJsonReport(scan: ApkScanResult): String {
        ...
        val fileObj = JSONObject().apply {
            ...
            // Sanitize internal filesystem paths/tokens from public export
            val sanitizedUri = if (scan.fileInfo.uri.startsWith("file://") || scan.fileInfo.uri.contains("/data/data/") || scan.fileInfo.uri.contains("/data/user/")) {
                scan.fileInfo.fileName
            } else {
                scan.fileInfo.uri
            }
            put("uri", sanitizedUri)
